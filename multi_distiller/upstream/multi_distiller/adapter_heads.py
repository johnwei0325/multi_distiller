...
class MLPAdapterHead(nn.Module):
    ...
    Transforms features from source size [B, T, H, C] to target size [B, T, H, C].
    Uses MLP to map channel and spatial sizes jointly, while preserving the overall shape.
    """
    ...
    def __init__(
        ...
        """Initialization function for MLPAdapterHead.
        ...
        # Define the MLP adapter
        self.adapter = nn.Sequential(
            nn.LayerNorm(source_channel_size),  # Apply LayerNorm over the channel dimension
            Rearrange('b t h c -> b t (h c)'),  # Flatten H and C into one dimension
            nn.Linear(source_channel_size * source_size[-2], hidden_dim),  # MLP layer
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),  # Another MLP layer
            nn.ReLU(),
            nn.Linear(hidden_dim, source_channel_size * source_size[-2]),  # Map back to original size
            Rearrange('b t (h c) -> b t h c'),  # Reshape back to [B, T, H, C]
            nn.LayerNorm(source_channel_size)  # Apply LayerNorm again
        )
        
        # Length adjuster (if needed, can adapt this for MLP as well)
        self.length_adjuster = nn.Sequential(
            nn.LayerNorm(source_channel_size),
            Rearrange('b t h c -> b t (h c)'),
            nn.Linear(source_channel_size * source_size[-2], source_channel_size * source_size[-2]),
            nn.ReLU(),
            nn.Linear(source_channel_size * source_size[-2], self.target_length * source_channel_size),
            Rearrange('b t (h c) -> b t h c'),
        )
    ...
    def grad_divide_hook(self, module, grad_input, grad_output):
        ...
        factor = 5  # The factor by which to divide the gradients
        if len(grad_input) == 1:  # Typically for nn.Linear
            return (grad_input[0] / factor,)
        else:
            return tuple(grad / factor if grad is not None else None for grad in grad_input)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass for MLPAdapterHead.
